...
The commit is prepared separately on `refactor/store-ui-mdx`. It preserves the existing application font imports and merchant tokens. The rewrite's antialiasing, balanced headings, explicit heading-font fallback, and reduced-motion rules remain in the original application stylesheet. Package manifests and the Bun lock contain only the dependencies and exports required by this rewrite. The original shared checkout remains unchanged by commit preparation.
...
| Isolated shell interactions; `tests/e2e/merchant-ui-fixtures.spec.ts:166` | Observed passing checks include closed mobile navigation absence, initial close-button focus, Escape focus restoration, theme switching/preload release, desktop resize dismissal, labeled account select, and recovery callback. These fixtures do not exercise authenticated commerce. |
...
The durable test sources are the account overview and navigation tests, admin navigation/dashboard state tests, and `tests/e2e/merchant-ui-fixtures.spec.ts`. Matrix results and representative screenshots are saved in [evidence/store-ui](./evidence/store-ui/). Temporary runners and detailed logs remain under `/tmp`. These are local verification artifacts, not production evidence. The synthetic fixture is gated by the existing merchant-fixture route controls and must not be mistaken for live merchant data.
...
The final official UI run used `/tmp/86d-store-ui-docker-e2e.sh`; its log is `/tmp/86d-store-ui-e2e.mhO3Bw/playwright.log`. The temporary override enabled fixtures and set the exact local authentication URL. The owned stack and volumes were removed afterward (`cleanup_status=0`), and the temporary development server was stopped. The usual Playwright global setup wrote its ignored local session artifact.

## Isolated commit verification

Commit preparation revalidated the rewrite in `.worktrees/86d-store-ui-mdx`, on `refactor/store-ui-mdx`, from base `a5b1aa1895790aff162f2124fd0cd3d7b90cc236`. This checkout excludes the concurrent font/token migration. The existing contrast test now checks the homepage's full-opacity supporting copy at the normal-text 4.5:1 threshold; the original light/dark semantic-token checks remain intact.

The ordered frozen registry, typecheck, lint, full test, Docker build, and Docker smoke gates passed. Typechecking completed 115 tasks; lint checked 4,658 files; tests completed 117 workspace tasks and 68 repository tests. Logs are `/tmp/86d-ui-commit-{frozen,typecheck,check,test,docker-build,docker-verify}.log`.

The eight official UI browser tests also passed in 15.2s against `86d-store:ui-commit-20260908`, image ID `sha256:db656ed222a749659c574d07bf044bd41316048c68e5f40533903d420e241ca4`. This independently verifies the selected dashboard, navigation, theme, and recovery cases with the original font/token system. The real sign-in setup remained enabled. The runner used `/tmp/86d-store-ui-isolated-e2e.sh`; logs are in `/tmp/86d-store-ui-e2e.jDJCyl/`. Both the tests and stack cleanup exited 0. The earlier screenshots remain captures of the combined checkout.
...
**Unverified beside this verdict:** the full commerce Playwright suite, every interaction state on every real data route, and live commerce workflows. The six required repository gates and eight selected official UI tests passed. Neither local source review nor fixture success advances capability maturity, proves checkout/order continuity, or authorizes remote publication. Commits remain local; no remote publication was performed.
