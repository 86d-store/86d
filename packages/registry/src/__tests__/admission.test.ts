...
describe("evaluateModuleAdmission", () => {
	it("denies a generated Module that was not explicitly selected", () => {
		...
		expect(decision).toEqual({
			...
			message:
				'Module "@86d-store/cart" is not selected by config.modules and cannot be enabled.',
		});
	});

	it("blocks new enablement of a Deprecated Module with transition guidance", () => {
		const decision = evaluateModuleAdmission({
			moduleName: "@86d-store/legacy-payments",
			maturity: "deprecated",
			modules: ["@86d-store/legacy-payments"],
		});
		...
		expect(decision).toEqual({
			...
			message:
				'Deprecated Module "@86d-store/legacy-payments" cannot be newly enabled. Follow its documented transition before changing config.modules.',
		});
	});
});
