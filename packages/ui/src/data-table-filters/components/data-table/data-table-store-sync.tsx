 ...
 *
 * Requires a DataTableStoreProvider ancestor (same as useFilterActions).
 */
import { useStoreContext } from "~/data-table-filters/lib/store/context";
import { useFilterActions } from "~/data-table-filters/lib/store/hooks/use-filter-actions";
import { isStateEqual } from "~/data-table-filters/lib/store/schema/serialization";
...
export function useDataTableStoreSync() {
	...
	const isInitialMount = useRef(true) as MutableRefObject<boolean>;
	...
	useEffect(() => {
		if (isInitialMount.current) {
			...
			const byosState = adapter.getSnapshot().state as Record<string, unknown>;
	...
	useEffect(() => {
		...
		const selectedKeys = Object.keys(rowSelection);
		const newUuid = selectedKeys.length > 0 ? (selectedKeys[0] ?? null) : null;
