...
import { CircleNotchIcon as LoaderCircle } from "@phosphor-icons/react/dist/ssr";
import type {
...
import {
	...
	TableHeader,
	TableRow,
} from "~/data-table-filters/components/custom/table";
import { DataTableFilterControls } from "~/data-table-filters/components/data-table/data-table-filter-controls";
import { DataTableFilterRail } from "~/data-table-filters/components/data-table/data-table-filter-rail";
import { DataTableProvider } from "~/data-table-filters/components/data-table/data-table-provider";
import { DataTableResetButton } from "~/data-table-filters/components/data-table/data-table-reset-button";
import { DataTableToolbar } from "~/data-table-filters/components/data-table/data-table-toolbar"; // TODO: check where to put this
import type { DataTableFilterField } from "~/data-table-filters/components/data-table/types";
import {
	canLoadMore,
	columnSizeStyle,
} from "~/data-table-filters/components/data-table/utils";
import { useHotKey } from "~/data-table-filters/hooks/use-hot-key";
import { useLocalStorage } from "~/data-table-filters/hooks/use-local-storage";
import {
	getColumnOrderKey,
	getColumnVisibilityKey,
} from "~/data-table-filters/lib/constants/local-storage";
import { formatCompactNumber } from "~/data-table-filters/lib/format";
import { useFilterActions } from "~/data-table-filters/lib/store/hooks/use-filter-actions";
import { useFilterState } from "~/data-table-filters/lib/store/hooks/use-filter-state";
import {
	type DataTableFeatures,
	dataTableFeatures,
} from "~/data-table-filters/lib/table/features";
...
export interface DataTableInfiniteProps<TData extends RowData> {
	...
	renderLiveRow?: (props?: { row: Row<DataTableFeatures, TData> }) => ReactNode;
...
export function DataTableInfinite<TData extends RowData>({
	...
	return (
      ...
      */}
			...
			>
				<FilterPanel>
					<div className="shrink-0 border-border border-b bg-background p-2">
				...
				<div
					className={cn(
						"relative flex h-full max-w-full flex-1 flex-col border-border",
				...
				>
					...
					<div
						className={cn(
							"flex shrink-0 flex-col gap-4 bg-background p-2 pb-4",
					...
					<div className="z-0 min-h-0 flex-1">
						...
						>
										...
										{headerGroup.headers.map((header) => {
											return (
												<TableHead
													...
													className={cn(
														"relative select-none truncate border-border border-b last:[&>.cursor-col-resize]:opacity-0",
												...
												>
													...
													{header.column.getCanResize() && (
														<button
															...
															className={cn(
																"user-select-none absolute top-0 -right-2 z-10 flex h-full w-4 cursor-col-resize touch-none justify-center border-0 bg-transparent p-0",
							...
							<TableBody
								id="content"
								tabIndex={-1}
								className="outline-none outline-1 outline-primary -outline-offset-1 transition-colors focus-visible:outline-solid"
...
function DataTableBodyRow<TData extends RowData>({
	 ...
	 */
	const isDetail = useFilterState<Record<string, unknown>, boolean>((s) =>
		Boolean(isMultiSelect && s.uuid === row.id),
	);
	...
	return (
		<TableRow
			...
			data-detail={isDetail === true ? "" : undefined}
			...
			className={cn(
				...
				"outline-none outline-1 outline-primary -outline-offset-1 transition-colors focus-visible:bg-muted/50 focus-visible:outline-solid",
