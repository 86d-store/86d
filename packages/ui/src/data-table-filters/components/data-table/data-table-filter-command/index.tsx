...
} from "~/data-table-filters/components/data-table/data-table-filter-command/utils";
...
import { getCommandHistoryKey } from "~/data-table-filters/lib/constants/local-storage";
import { formatCompactNumber } from "~/data-table-filters/lib/format";
import type { SchemaDefinition } from "~/data-table-filters/lib/store/schema/types";
...
export function DataTableFilterCommand({
	...
	const isSerializingRef = useRef(false) as MutableRefObject<boolean>;
		...
		<div>
          ...
          one state only. */}
			...
			>
				...
				<Kbd className="ml-auto text-muted-foreground">
			...
			>
				...
				>
					...
					<CommandPrimitive.Input
						...
						className="flex h-full w-full bg-transparent text-foreground text-sm outline-hidden placeholder:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50"
					/>
				</div>
				<div className="relative">
					<div
						className={cn(
							"absolute top-2 z-10 w-full animate-in overflow-hidden border bg-popover text-popover-foreground shadow-md outline-hidden",
							boxRadiusClassName,
						)}
					>
							...
							<CommandGroup heading="Query">
								{filterFields.map((field) => {
									...
									const options = getFieldOptions({
										field,
										...(facetedValue != null ? { facetedValue } : {}),
									});
									...
									.map((item) => {
										return (
											...
											>
												...
												<span
													data-slot="command-shortcut"
													className="ml-auto truncate text-muted-foreground/80 group-aria-selected:block"
												>
												...
												<button
													...
													className={cn(
														"ml-1 hidden p-0.5 hover:bg-background group-aria-selected:block",
														boxRadiusClassName,
													)}
												>
													<X className="h-4 w-4" />
