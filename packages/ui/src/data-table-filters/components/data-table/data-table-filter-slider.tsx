...
import type { DataTableSliderFilterField } from "~/data-table-filters/components/data-table/types";
import { useDebounce } from "~/data-table-filters/hooks/use-debounce";
import { isArrayOfNumbers } from "~/data-table-filters/lib/is-array";
import {
	InputGroup,
	InputGroupAddon,
	InputGroupInput,
...
export function DataTableFilterSlider<TData>({
	...
	}, [debouncedInput, column?.setFilterValue]);
		...
		<div className="grid gap-2">
			...
			<Slider
				...
				onValueChange={(values) =>
					setInput(Array.isArray(values) ? [...values] : [values, values])
				}
			/>
		</div>
	);
