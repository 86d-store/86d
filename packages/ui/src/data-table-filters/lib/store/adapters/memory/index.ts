...
import type { InternalStoreAdapter } from "~/data-table-filters/lib/store/adapter/types";
...
export function useMemoryAdapter<T extends Record<string, unknown>>(
	...
	const pausedRef = useRef(false) as MutableRefObject<boolean>;
	const pendingRef = useRef<Partial<T> | null>(
		null,
	) as MutableRefObject<Partial<T> | null>;
