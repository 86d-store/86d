...
import type { FacetMetadataSchema } from "~/data-table-filters/lib/data-table/types";
...
export function getFacetedUniqueValuesFlattened<
	// biome-ignore lint/suspicious/noExplicitAny: mirrors TanStack facetedUniqueValues slot typing
	TData extends RowData = any,
>(): (
	// biome-ignore lint/suspicious/noExplicitAny: features erased at the faceted slot boundary
	table: TTable<any, TData>,
	columnId: string,
	// biome-ignore lint/suspicious/noExplicitAny: Map key type matches TanStack's faceted map
) => () => Map<any, number> {
	return (table, columnId) => {
		return () => {
			// biome-ignore lint/suspicious/noExplicitAny: RowModel features match the erased table
