import { createHash } from "node:crypto";
import {
	inventoryStockAdjustOutcomeSchema,
	inventoryStockAdjustTransportSchema,
} from "@86d-app/inventory";
import type { CommandPrincipal } from "@86d-app/runtime/command";
import type { Session } from "auth";
import env from "env";
import type { NextRequest } from "next/server";
import { NextResponse } from "next/server";
import {
	createInventoryCommandExecutor,
	createStoreAdminInventoryAuthority,
	inventoryStockAdjustCommandReference,
} from "./inventory-command-executor";

function commandFailureStatus(code: string) {
	switch (code) {
		case "invalid_request":
		case "invalid_input":
			return 400;
		case "unauthenticated":
			return 401;
		case "forbidden":
			return 403;
		case "target_not_found":
			return 404;
		case "idempotency_conflict":
			return 409;
		case "temporarily_unavailable":
			return 503;
		default:
			return 422;
	}
}

function failureResponse(code: string, message: string, status: number) {
	return NextResponse.json({ error: { code, message } }, { status });
}

/** Authenticated edge adapter for the first production Store Command. */
export async function handleInventoryStockAdjustCommand(
	request: NextRequest,
	session: Session,
) {
	const parsed = inventoryStockAdjustTransportSchema.safeParse(
		await request.json().catch(() => undefined),
	);
	if (!parsed.success) {
		return failureResponse(
			"INVALID_INVENTORY_ADJUSTMENT",
			"The inventory adjustment request is invalid.",
			400,
		);
	}

	if (!env.BETTER_AUTH_SECRET) {
		return failureResponse(
			"COMMAND_DIGEST_KEY_UNAVAILABLE",
			"Store Commands are unavailable until the Store authentication secret is configured.",
			503,
		);
	}

	const digestKey = createHash("sha256")
		.update("86d.store-command.digest.v1\0")
		.update(env.BETTER_AUTH_SECRET)
		.digest("hex");
	const principal = {
		type: "session",
		credentialId: session.session.id,
		sessionId: session.session.id,
	} satisfies CommandPrincipal;
	const executor = await createInventoryCommandExecutor({
		authority: createStoreAdminInventoryAuthority({
			storeId: env.STORE_ID,
			userId: session.user.id,
			sessionId: session.session.id,
			role: session.user.role,
		}),
		digestKey,
	});
	const result = await executor.execute(
		{
			command: inventoryStockAdjustCommandReference,
			idempotencyKey: parsed.data.idempotencyKey,
			target: { type: "store", id: env.STORE_ID },
			input: {
				productId: parsed.data.productId,
				...(parsed.data.variantId ? { variantId: parsed.data.variantId } : {}),
				...(parsed.data.locationId
					? { locationId: parsed.data.locationId }
					: {}),
				delta: parsed.data.delta,
				correlationId: parsed.data.idempotencyKey,
			},
		},
		{ principal },
	);

	if (!result.ok) {
		return failureResponse(
			result.failure.code.toUpperCase(),
			result.failure.message,
			commandFailureStatus(result.failure.code),
		);
	}
	if (result.receipt.status !== "succeeded") {
		return failureResponse(
			"COMMAND_IN_PROGRESS",
			"The Inventory Command has not reached a terminal result.",
			503,
		);
	}

	const adjustment = inventoryStockAdjustOutcomeSchema.safeParse(
		result.receipt.result,
	);
	if (!adjustment.success) {
		return failureResponse(
			"INVALID_COMMAND_RESULT",
			"The Inventory Command returned an invalid result.",
			500,
		);
	}

	return NextResponse.json({
		adjustment: adjustment.data,
		executionId: result.receipt.executionId,
		replayed: result.receipt.replayed,
	});
}
